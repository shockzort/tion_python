...
def test_run_scenario(scenarist, capsys):
    ...
    scenario_id = scenarist.create_scenario(
        Scenario(
            ...
            trigger_type="manual",
            trigger_params={},
            action_params={"message": "Scenario executed"}
        )
    )
    
    result = scenarist.run_scenario(scenario_id)
    assert result
    
    captured = capsys.readouterr()
    assert "Running scenario" in captured.out
