...
## Шаг 0. TLS: IP-сертификат или DuckDNS (15 минут, план §6)

Яндексу нужен публичный HTTPS с валидным сертификатом. Два пути:

**Путь A — IP-сертификат Let's Encrypt (без домена, предпочтение владельца).**
...
Если консоль принимает URL с IP — путь A рабочий; если требует домен — путь B.

**Путь B — бесплатный поддомен DuckDNS (основной, если A не прошёл).**
...
Дальше `<host>` = IP или поддомен в зависимости от пути.
