...
2. Обязательные поля: название **минимум из двух слов** («Easy Breezy»
   подходит), подзаголовок, описание, **иконка PNG/JPG не меньше 224×224**
   (подойдёт `ui/public/pwa-512.png`).
3. Раздел **«Доступ»** → тип **«Приватный»**: по докам «приватный навык не
   отправляется на модерацию, а сразу публикуется» и появляется в приложении
   «Дом с Алисой» только у создателя. Публикация нужна: голосовое управление
   и сервис уведомлений (наши callbacks) работают **только после публикации**.
   Черновик в списке производителей приложения НЕ виден — до публикации
   линковку и команды можно проверить во вкладке «Тестирование» консоли
   (там встроен интерфейс «Дома с Алисой» и лог запросов).
4. **Endpoint URL**: `https://<host>` — **без** `/v1.0`: платформа сама
   добавляет `/v1.0/...` к адресу (проверка доступности — `HEAD <host>/v1.0`,
   запросы — `<host>/v1.0/user/devices` и т.д.).
5. **Связка аккаунтов (OAuth)**:
...
6. Сохранить черновик. Из консоли выписать:
